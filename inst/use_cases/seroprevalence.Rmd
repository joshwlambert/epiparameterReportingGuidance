---
title: "Example use case: the importance of clear reporting of seroprevalence estimates"
...
Using the incorrect denominator could have important consequences for future planning and control strategies. Consider a population of `r pop_size` people. The basic reproduction number of this novel coronavirus is estimated at around `r r0`, implying a herd immunity threshold (HIT) of `r hit*100`%. \
If it is (incorrectly) assumed that seroprevalence is `r round(binom.confint(x = positive_neutralisation,n = 2*positive_elisa,method="exact")$mean,3) * 100`%, this implies that `r round((hit - sero_diff)*pop_size)` members of the population require immunity before herd immunity is reached. However, it is actually `r round((hit - binom.confint(x = positive_neutralisation,n = total_tests,method="exact")$mean )*pop_size)` members of the population requiring immunity to reach this threshold (an additional `r round(sero_diff*pop_size)` people) under the correctly specified seroprevalence estimate of `r round(binom.confint(x = positive_neutralisation,n = total_tests,method="exact")$mean,3) * 100`%. \
If immunity were to be required through vaccination, then an additional `r round(sero_diff*pop_size)` vaccines would be required. In the absence of a vaccine, e.g. if immunity were to be acquired via natural infection, assuming an overall case hospitalisation ratio (CHR) of `r chr*100`% and case fatality ratio (CFR) of `r cfr*100`%, this suggests an additional `r round(chr*sero_diff*pop_size)` hospitalisations and `r round(cfr*sero_diff*pop_size)` deaths than could be expected under the assumption of a seroprevalence of `r round(binom.confint(x = positive_neutralisation,n = 2*positive_elisa,method="exact")$mean,3) * 100`%
